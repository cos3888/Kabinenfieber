...
code=code.replace(/\n  if \(document\.readyState === 'loading'\) \{/,`\n  window.KFTest={AppState,startNewCareer,nextCalendarSlot,fixturesForSlot,advanceCareerRound,runCalendarSimulationUntil,prepareCareerSlotLifecycle,financeEventSum,financePlacementBonus,recordLicenceDecisionForSeasonEnd,scheduleFutureMovement,targetSlotForSimulation};\n  if (document.readyState === 'loading') {`);
vm.runInContext(code,context,{filename:'src/app.bundle.js'});if(document.cb)document.cb();
const T=windowObj.KFTest;
check('Runtime enthaelt den KF_0.25.4-Fix und laeuft aktuell als KF_0.26.2',read('src/app.bundle.js').includes("var KF_VERSION = '0.27.1';")&&read('index.html').includes('KF_0.27.1')&&JSON.parse(read('package.json')).version==='0.27.1',{});
...
T.startNewCareer();
const base=JSON.parse(JSON.stringify(T.AppState.world));
...
const normal=JSON.parse(JSON.stringify(base)), fast=JSON.parse(JSON.stringify(base));
...
function salaryState(w){return w.clubs.order.map(id=>({id,amount:T.financeEventSum(w,id,season,ev=>ev.type==='salaryExpense'),slots:((((w.clubFinances||{}).byClub||{})[id]||{}).financeEvents||[]).filter(ev=>ev.type==='salaryExpense').reduce((n,ev)=>n+Number(ev.slotsApplied||0),0)}));}
...
const fresh=JSON.parse(JSON.stringify(base)), sampleClub=fresh.clubs.byId[fresh.clubs.order[0]], sampleFinance=fresh.clubFinances.byClub[sampleClub.id];
...
for(const pos of [1,5,12,18]){
  const w=JSON.parse(JSON.stringify(base)), cid=w.clubs.order[0], club=w.clubs.byId[cid], oldOrder=w.clubs.order.slice();w.clubs.order=[cid];
  ...
  const count=((((w.clubFinances||{}).byClub||{})[cid]||{}).financeEvents||[]).filter(ev=>ev.type==='placementBonus'&&Number(ev.seasonId||0)===season).length;
