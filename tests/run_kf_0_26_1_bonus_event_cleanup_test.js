...
check('Runtime meldet KF_0.29.3',code.includes("var KF_VERSION = '0.29.3';")&&read('index.html').includes('KF_0.29.3')&&JSON.parse(read('package.json')).version==='0.29.3');
...
const clubFinance=T.financeEventsForSeason(w,clubId,Number(w.meta.seasonNumber||1));
...
const beforeReloadCount=T.financeEventsForSeason(reloaded,clubId,Number(reloaded.meta.seasonNumber||1)).length;
const paidReload=T.kf021ProcessMatchContractBonuses(reloaded,match);
const afterReloadCount=T.financeEventsForSeason(reloaded,clubId,Number(reloaded.meta.seasonNumber||1)).length;
...
const seasonEvents=T.financeEventsForSeason(w,clubId,season).filter(ev=>ev.eventKey===seasonKey);
...
const loanWorld=JSON.parse(JSON.stringify(w));loanWorld.meta.id=String(w.meta.id)+'_loan_test';T.migrateWorldDataTruthToCurrent(loanWorld);for(const cid of w.clubs.order){T.CurrentSeasonFinanceRepository.replaceClubEvents(loanWorld,cid,season,T.financeEventsForSeason(w,cid,season).map(ev=>JSON.parse(JSON.stringify(ev))));}const loanPlayer=loanWorld.players.byId[playerId];
...
const loanEvents=[...T.financeEventsForSeason(loanWorld,clubId,season),...T.financeEventsForSeason(loanWorld,club2,season)].filter(ev=>ev.eventKey===loanKey);
...
const transferReload=JSON.parse(JSON.stringify(w));transferReload.meta.id=String(w.meta.id)+'_transfer_reload_test';T.migrateWorldDataTruthToCurrent(transferReload);for(const cid of w.clubs.order){T.CurrentSeasonFinanceRepository.replaceClubEvents(transferReload,cid,season,T.financeEventsForSeason(w,cid,season).map(ev=>JSON.parse(JSON.stringify(ev))));}
const pCountBefore=T.financeEventsForSeason(transferReload,club2,season).length,bCountBefore=T.financeEventsForSeason(transferReload,club3,season).length;
...
const pCountAfter=T.financeEventsForSeason(transferReload,club2,season).length,bCountAfter=T.financeEventsForSeason(transferReload,club3,season).length;
check('Transferklausel bleibt nach Reload ohne Doppelbuchung',pCountBefore===pCountAfter&&bCountBefore===bCountAfter,{pCountBefore,pCountAfter,bCountBefore,bCountAfter});
...
const migratedPay=T.financeEventsForSeason(legacy,club2,season).find(ev=>ev.id==='legacy-pay'),migratedIncome=T.financeEventsForSeason(legacy,club3,season).find(ev=>ev.id==='legacy-income');
check('0.26.0-Migration überträgt Legacy-Transferkey und leert Bonus-Historie',legacy.meta.schemaVersion==='kf-core-0.27.2'&&legacy.history.bonusEvents.length===0&&migratedPay.eventKey===legacyKey&&migratedIncome.eventKey===legacyKey,{migration:migration.bonusMigration,migratedPay,migratedIncome});
