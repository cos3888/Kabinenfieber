...
const s3IncomeMissing=aiIds.filter(id=>{const cs=T.sponsorContractsForClub(w,id,3);const events=((((w.clubFinances||{}).byClub||{})[id]||{}).financeEvents||[]).filter(ev=>ev&&ev.type==='sponsorIncome'&&Number(ev.seasonId||0)===3);return events.length!==cs.length;});
