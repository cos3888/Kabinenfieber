...
export function formationStarterPositions(formationRecord) {
  return (formationRecord?.occupiedSlots || [])
    .map(slotId => formationRecord?.positionMap?.[slotId])
    .filter(Boolean);
}

export function buildBenchAndReserveTemplate(starterPositions, rng, targetExtraCount) {
  const counts = starterPositions.reduce((acc, positionCode) => {
    acc[positionCode] = (acc[positionCode] || 0) + 1;
    return acc;
  }, {});

  const extras = ['TW'];
  const mustHaveBackups = ['IV', 'LV', 'RV', 'DM', 'ZM', 'OM', 'ST'];
  mustHaveBackups.forEach(positionCode => {
    if ((counts[positionCode] || 0) > 0) extras.push(positionCode);
  });

  ['LM', 'RM', 'LF', 'RF', 'HS'].forEach(positionCode => {
    if ((counts[positionCode] || 0) > 0 && extras.length < targetExtraCount) extras.push(positionCode);
  });

  const weightedPool = [];
  Object.entries(counts).forEach(([positionCode, count]) => {
    for (let i = 0; i < Math.max(1, Number(count) || 1); i += 1) weightedPool.push(positionCode);
  });

  if (!weightedPool.length) weightedPool.push('ZM', 'IV', 'ST', 'LV', 'RV', 'OM');

  while (extras.length < targetExtraCount) {
    extras.push(weightedPick(weightedPool, () => 1, rng));
  }

  return extras.slice(0, targetExtraCount);
}

export function buildSquadTemplateForFormation(formationRecord, rng) {
  const starters = formationStarterPositions(formationRecord);
  const targetSize = randomInt(rng, ...DB2_START_RULES.squad.targetSizeRange);
  const targetExtraCount = Math.max(0, targetSize - starters.length);
  const extras = buildBenchAndReserveTemplate(starters, rng, targetExtraCount);
  return [...starters, ...extras].slice(0, targetSize);
}

export function chooseMainPositionForGroup(groupKey, rng) {
  const exactPositionCodes = new Set(['TW', ...Object.keys(DB2_SECONDARY_POSITION_MAP)]);
  if (exactPositionCodes.has(groupKey)) return groupKey;
